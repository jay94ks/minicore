#!/usr/bin/env bash
# 유저랜드(minicore/init, minicore/devmgr)를 빌드해 CPIO(newc) 아카이브로
# 묶는다 - QEMU/GRUB에 module2로 실어 실제 유저 프로세스가 뜨는
# 시나리오(SP-68182FBD/PN-D3C05C0B)를 재현하기 위한 개발 도구.
#
...
# "init"/"devmgr")을 GNU cpio -H newc로 그대로 만족한다(실측 확인 -
# 이 파일 작성 시 xxd/python으로 헤더 바이트까지 직접 검증했다).
#
# 사용법:
...
# 한다(경로 접두어 없이 "init"/"devmgr"/"pubreg"/"fs") - cpio 아카이브
# 안에도 이 이름 그대로 들어가도록 스테이징 디렉터리 안에서 상대경로로
# 넘긴다. pubreg(PN-185406F6, 2026-09-17)/fs(PN-452FF696, 2026-09-17)
# 추가 - init/devmgr과 동일한 이유.
...
echo "initrd 생성 완료: ${OUT_PATH} ($(stat -c%s "${OUT_PATH}" 2>/dev/null || stat -f%z "${OUT_PATH}") bytes, init+devmgr+pubreg+fs)"
