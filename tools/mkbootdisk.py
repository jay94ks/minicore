...
§M12/§M13, ADR-131/152). tools/mkinitrd.py가 만드는 MCPACK initrd(커널이
자신에 심는 것)와는 완전히 별개다 — 이건 virtio-blk 장치로 붙는 진짜
디스크 이미지이고, initrun 자신의 임베디드 cpio 리더(init/initrun/
cpio_reader.cpp)가 읽는 포맷을 그대로 따라야 한다.

사용법:
    mkbootdisk.py <출력경로> --service=<이름>=<elf경로> [...]
                             [--depends=<이름>:<의존 서비스 이름>] [...]

각 --service마다 두 엔트리를 담는다:
    bin/<이름>              — ELF 바이너리 그대로
    lib/NNN-<이름>.ini      — [<이름>]\\nexec=bin/<이름>\\n
                              (--depends로 지정됐으면 depends=<이름>\\n도 추가)
(NNN은 나열 순서대로 000, 001, ... — ADR-131 §결정5: "파일명순=실행순"
이므로 --service 인자 순서가 곧 기동 순서다 — 의존 서비스는 반드시
그보다 먼저 나열해야 한다.)
...
def main(argv):
    ...
    for arg in argv[2:]:
        if arg.startswith("--service="):
            ...
            spec = arg[len("--depends="):]
            if ":" not in spec:
                print(f"잘못된 --depends 형식(이름:의존이름이어야 함): {spec}", file=sys.stderr)
                return 1
