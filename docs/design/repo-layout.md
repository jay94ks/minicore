...
│   ├── CMakeLists.txt
│   ├── include/                # 유저랜드와 공유하는 커널 ABI 헤더 (syscall 번호 등)
...
│
├── libk/                        # 커널·시스템 서버 공용 프리스탠딩 코어 라이브러리 (ADR-010)
│   └── include/libk/            # result, optional, span, intrusive_list, atomic 래퍼 등
│
├── init/
...
- 서버/유저랜드 컴포넌트 목록은 최상위에서 "이 arch에서 무엇을 빌드하는가" 배열로
  ...
  컴파일 검증한다 (ADR-009).
- `libk`는 커널과 시스템 서버가 함께 링크하는 헤더 전용/정적 라이브러리로 취급한다
