...
| OPEN-59 | ADR-151이 IPC 메시지 구조체는 서로 다른 유저 주소공간 사이에서도 안전하게 옮기지만, `pages[]`가 가리키는 실제 데이터 버퍼 내용은 여전히 같은 주소공간 전제로만 안전하다(M13은 32바이트 이하 값을 regs[]로만 주고받아 이 경로를 피해 갔다) — 실제로 페이지 단위 큰 데이터를 서로 다른 프로세스 사이에 옮겨야 하는 시점(파일 내용이 커지는 M14 이후 등)에 같은 방식(주소공간별 페이지 번역)으로 재검토 필요. 서비스 간 엔드포인트 탐색(등록/이름 서비스가 아직 없어 스폰 시점 캐패빌리티 주입 — ADR-152 — 으로만 임시 해결한 것)도 실제 이름 서비스가 생기면 재검토 대상 | ADR-151, ADR-152 | [kernel-ipc-objects.md](kernel-ipc-objects.md) |
