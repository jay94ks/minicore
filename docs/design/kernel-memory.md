...
- **결정**: ADR-052가 "락 순서 규칙은 각 서브시스템 코드를 작성하며
  ...
  **M34 갱신**([kernel-scheduler.md](kernel-scheduler.md) ADR-209) —
  `run_queue::lock`/`klog::g_log_lock`(둘 다 `irq_safe<spinlock>`)이
  "이론상 여러 코어에서 경합 가능"에서 **"실제로 상시 경합"**으로
  바뀌었다(AP가 진짜로 유저 밴드에 참여하고 debug_log syscall도
  여러 코어에서 동시에 호출된다) — 그 과정에서 `irq_safe<Lock>`
  자체의 진짜 데이터 경합(뮤텍스를 잡기 **전에** 공유 `state_`
  필드에 irq 상태를 저장하던 순서 버그)을 발견해 고쳤다(ADR-209
  §실행 중 발견 1). 이 표의 두 락 모두 그 수정으로 다시 정확하다.
  위 `handle_table` 결여는 이번에도 해소되지 않는다 — busy/counter
  데모(IPC 없음)는 이 경로를 건드리지 않는다.
...
- **결정**:
  ...
  2. **캐패빌리티 슬롯 영역을 균일한 고정 크기 슬롯의 표로 확정**:
     ...
     **예산**으로 못박는다. 슬롯 `N`은 `k_user_stack_top + N *
     ...
     | 4 | `[0x400000, 0x500000)` | IPC `pages[]` 매핑(슬롯 시작 지점, 최대 4페이지 — M56/ADR-229부터 이 1MiB 전체를 스레드별 16KiB 부분 슬롯(최대 64개)으로 다시 나눈다, 바깥쪽 경계는 그대로) | ADR-159(계획), ADR-229 |
