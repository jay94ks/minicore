...
| [kernel-memory.md](kernel-memory.md) | ADR-012, 016, 024, 033, 036, 051, 052, 054, 078, 104~108, 110, 121, 134, 136 | 메모리 할당, 쿼터, NUMA, 락 모델(실제 락 순서 표 포함), 가상메모리 레이아웃, 메모리 예약·회수·압박 신호, 스왑 압축, 압축 실행 위치(SWAPFS 서버), 유저 주소공간의 저지대 항등 매핑(GDT) 공유, 슬랩 헤더 16바이트 정렬 |
| [kernel-scheduler.md](kernel-scheduler.md) | ADR-014, 025, 027, 034, 035, 053, 055, 109, 124, 127, 133, 135, 137 | 우선순위 밴드, NUMA 런큐, 승격, 도네이션, 최근 스케줄링 빈도 측정, 스레드 영구 종료(sched::exit), FPU/SIMD 컨텍스트 스위칭(lazy XSAVE/AVX로 개정), AP 기동(INIT-SIPI-SIPI) 구현 세부, 워크 스틸링 범위 정정(커널 밴드도 노드 경계를 넘음) |
