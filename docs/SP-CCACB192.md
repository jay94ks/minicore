...
<!--
  ...
  status: review
  updatedAt: 2026-09-16T22:03:42.710Z
...
**권장**: (a) - tool 선언/등록·조회 메시지 모두 지금까지 설계된
스키마 예시(§2 tool 선언, §3 register/query)에 소수가 등장한 적이
없고, 정수만으로 충분해 보인다. 나중에 실제로 소수가 필요해지면
그때 (b)로 확장(파서 API 자체는 안 바뀜 - `onNumber` 콜백이 정수/
실수 겸용 값을 넘기도록만 설계해 두면 호환 확장 가능).
...
**권장**: (a) - 지금까지 설계 문서의 모든 예시(tool 이름/설명)가
ASCII다. 실제로 비ASCII 문자열이 필요해지면 그때 확장.
