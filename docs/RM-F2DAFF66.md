...
- **[점검 완료, 2026-09-24] `SP-CC2B18C6`(UEFI physicalBase 재배치
  설계, approved 2026-09-24) - §2/§3-0~§3-3의 "확정된 설계" 전 항목
  대조, 갭 없음** - §3에서 "구현 착수 전이라 보류"로 남겨 뒀던 항목을
  `PN-7FBF255A`가 이번 세션에 실제로 kMain 도달까지 성공시켜(commit
  `2e867f2`/`0865640`) 이제 대조 가능해졌다. §2(physicalBaseDelta
  공식) - `paging.cpp`/`kmain.cpp`/`page_frame_allocator.cpp` 호출부
  전부 구현+실측(GRUB/PVH delta=0 무회귀). §3-0~3-2(스테이지 로더
  절차) - `main.cpp`의 PT_LOAD 캡처/physicalBase 탐색(1GiB 제약+AP
  트램폴린 회피 명시적 검증 포함)/세그먼트 복사/페이지테이블 구성/
  CR3 전환/jmp 전부 구현+실측. §3-3(매직 마커) - `boot.S`의
  `boot_marker_magic`/`pml4`/`entry`/`saved_start_info`/`gdt64` 5필드
  전부 구현, 바이트 단위 실측 대조 완료. §4(BootInfo 전달 경로) -
  "새 필드 추가 vs 호출부에서 delta만 더해 넘기기, 두 방식 다
  가능(구현 디테일)"이라고 §4 스스로 명시해 뒀던 대로, 실제로는 후자
  (로컬 `physicalBaseDelta` 변수를 `Paging::init()`/
  `PageFrameAllocator::init()` 호출부에 직접 전달)를 택했다 -
  `BootInfo` 구조체에 새 필드를 안 넣은 게 §4 위반이 아니라 §4가
  이미 열어 둔 선택지 그대로임을 확인. §5(미결 사항) - 명시된 항목
  중 "PT_LOAD 파일/물리 오프셋 관계"/"1GiB 제약 실제 강제"/"AP
  트램폴린 상호작용"은 전부 코드로 구현+실측됐고, 유일하게 남은
  "UEFI 메모리맵/ACPI RSDP 전달"은 §4/§5 스스로 "이 설계가 명시적으로
  다루지 않는 것"으로 이미 분류해 둔 항목이라 이 SP 문서 자체의 갭이
  아니라 별도 후속 설계 대상(`DC-F196028B`/`QU-C5EC3A77`, 설계자
  답변 대기 중)으로 이미 분리돼 있다. **결론: 이 SP 문서가 확정한
  것 중 실제 코드에 반영 안 된 항목 없음** - 위 §3 첫머리의 보류
  메모는 이 항목으로 대체.
