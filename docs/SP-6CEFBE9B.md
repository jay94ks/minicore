...
<!--
  ...
  status: approved
  updatedAt: 2026-09-21T14:11:14.210Z
...
2. **재접근 감지 — [정정, 2026-09-18, 설계자 opinion] 폴트 기반이
   ...
   **[3차 정정, 2026-09-21, QU-02EF5205 설계자 재답변] 위 "재정정"은
   오적용이었음 - "읽기 전용+#PF"는 CoW 기법이지 이 스캔의 메커니즘이
   아니다**: 후속 질의(`QU-02EF5205`)로 확인한 설계자 답변 원문 -
   "'읽기 전용 재매핑 + #PF 트랩'은 CoW(Copy on write)를 구현하기
   위한 전략이고, 하드웨어 accessed 비트를 page frame 구조체에
   접근할 때 읽어가도록 만들면 돼." 즉 `QU-C0E6CB31`의 원 답변은
   **`fork()`의 CoW 인프라**(이미 `SP-6BEAE0C1` §2/§11에 설계돼
   있음 - 쓰기 시 폴트로 페이지를 실제 복사하는 표준 기법)를 가리킨
   것이었지, 이 스캔의 accessed-bit 관찰 메커니즘과는 무관했다 -
   두 메커니즘을 잘못 엮어 적용한 것이 바로 위 "재정정" 단락의
   오류다. **원래 설계(순수 PTE-walk로 하드웨어 Accessed 비트를 직접
   읽는다)가 맞았다** - 다시 그 결론으로 되돌린다: 스캔이 각 프레임의
   PTE(page frame 구조체가 가리키는 실제 매핑)에 접근할 때 하드웨어
   Accessed 비트를 그대로 읽고(있으면 `PG_ACCESSED` 반영 후 PTE
   쪽은 클리어) 그 자리에서 승격까지 처리한다 - 위 "**애초에 폴트로
   재접근을 감지한다는 전제 자체가 틀렸다... 유일한 방법은 3단계의
   스캔이 각 프레임의 PTE를 직접 walk해 하드웨어 Accessed 비트를
   읽는 것**" 문단이 최종 결론이다. §8-1.3도 이 되돌림을 반영했다.
...
- **FileBacked, 더러운(dirty) 경우**(로드 이후 쓰기가 있었음): 프레임을
  ...
  검증 불가) - `PN-452FF696`(fs 서비스) 완료 이후로 남긴다(§8-2 항목2
...
## 8. 스왑 회수 트리거/스캔 - 설계 제안 (§8-1, PN-4859FDE9, 2026-09-21)

[제안, 2026-09-21, `PN-4859FDE9`] §7.3이 "스캔이 언제/어떤 스레드에서
도는지"로 열어 뒀던 §8-1을 실제 설계안으로 채운다 - 구조/트리거
메커니즘은 아래처럼 제안하되, 정확한 숫자(워터마크 %, 배치 크기,
스캔 주기)는 여전히 실측 후 조정 대상으로 남긴다(RM-23F4B687 §4).
`QU-`(질의 등록 예정)로 설계자 확인을 구한다.

### 8-1.1 왜 동기적 "고갈 시 즉시 회수"가 아니라 별도 백그라운드 스레드인가

`PageFrameAllocator`의 "즉시 nullptr, 비블로킹" 고갈 정책
(`SP-D7013B26` §2.4)은 이 커널 전역 원칙이다 - `alloc()` 호출부가
할당 실패 시 **절대 슬립/대기하지 않는다**는 전제 위에 비동기
런타임(`AsyncTask`)/인터럽트 컨텍스트 코드 상당수가 서 있다. 회수
(eviction)는 본질적으로 "더러운 페이지를 swapfs에 다 쓸 때까지
기다려야 할 수 있는" 블로킹 동작이므로, **할당 실패 그 순간 동기적으로
회수를 시도하는 것은 이 전역 원칙과 정면으로 충돌한다.** 따라서 Linux의
`kswapd`와 동일한 결론 - 회수는 **별도의 백그라운드 실행 경로**에서
비동기로 미리 여유분을 만들어 두고, `alloc()` 자신은 그 결과(이미
회수돼 free-list에 돌아온 프레임)만 그대로 소비할 뿐 회수 자체를
기다리지 않는다. **[정정, 2026-09-21, 설계자 직접 지시] 이 "별도
경로"가 전용 KernelThread여서는 안 된다** - 아래 §8-1.2 참고.

### 8-1.2 [정정, 2026-09-21, 설계자 직접 지시] 실행 주체 - 전용 스레드 없음, `DelayedExecutionQueue` 콜백 재사용

**설계자 직접 지시(2026-09-21, 대화): "스캔 전용 Task를 만드는건
불허하는 내용이야."** 이 절이 원래 제안했던 "전용
`kMemScanKernelMain` KernelThread"는 폐기한다 - 사실 이 프로젝트는
정확히 이 문제
(주기적 백그라운드 작업을 어떻게 돌릴지)를 `SP-F15B4A63`(지연 실행
인프라) §3에서 이미 다뤘고, 그 결론이 바로 "전용 Task를 만들지
않는다"였다(`QU-A8C0CC2C` 설계자 답변: "DelayedExecutionQueue
전용으로 Task를 만들기 보다... 비동기 리액터를 활용하는 방향으로
가자" - 후보 (b) "전용 저빈도 커널 Task"는 명시적으로 폐기됨). 이
스캔도 그 정책을 그대로 따라야 했다 - devmgr/fs의 `kSpawnKernelThread`
선례를 잘못 끌어왔다(devmgr/fs는 완전한 서비스 프로세스 대체물이라
전용 스레드가 맞지만, 이 스캔은 SP-F15B4A63이 이미 다룬 "가벼운
주기적 콜백" 범주다).

**갱신된 설계**: 스캔은 **`DelayedExecutionQueue`(`SP-F15B4A63` §2)에
등록되는 1회성 콜백**으로 구현한다 - 새 Task/KernelThread를 전혀
만들지 않는다. 이 콜백은 `SP-F15B4A63` §3이 확정한 실행 경로
(`Scheduler::runLoop()`의 idle 폴백에 인라인된 AsyncReactor 로직이
매 사이클 `DelayedExecutionQueue::pump()`를 호출)를 통해 실행되며,
실행이 끝날 때마다 스스로를 다시 "N틱 뒤"로 재등록(self-rearm)해
주기적 동작을 흉내낸다 - 별도 스레드의 sleep/wake 루프가 아니라
"한 번 실행하고 다음 실행을 다시 큐에 등록"하는 콜백 체인이다.
코어별로 나누지 않고 **콜백 하나만** 유지한다(§8-1.5 실측 후 재검토
대상은 그대로 유지).

### 8-1.3 깨우는 조건 - 두 워터마크(Linux 관례) + 순수 폴링, 새 동기화 프리미티브 없음

```cpp
// page_frame_allocator.h에 추가 제안
enum class MemoryWatermark : uint32_t {
    Low,   // 이 밑으로 떨어지면 스캔 콜백이 "적극적으로" 회수 시작
    High,  // 회수가 이 위로 올라오면 멈추고 다시 잠듦(히스테리시스 -
           // Low에서 딱 Low 위로만 회수하면 계속 깨었다 잠들었다 진동함)
};
// 정확한 비율(예: 전체 프레임의 5%/10%)은 실측 후 조정 - 지금은
// PageFrameAllocator::freePageCount()/totalPageCount() 비율로 계산.
```

- **[정정, 2026-09-21] 순수 폴링, 새 IPC/깨우기 메커니즘 도입 안 함,
  전용 스레드도 없음** - 위 §8-1.2 갱신대로 이 스캔은 전용 스레드의
  sleep/wake 루프가 아니라 `DelayedExecutionQueue`(`SP-F15B4A63`)에
  등록되는 자기재등록형 콜백이다 - 매 실행이 끝나면 콜백 스스로
  "N틱 뒤 다시 실행해 달라"를 큐에 재등록한다(새 스케줄러 프리미티브
  불필요, 새 스레드도 불필요). `SP-677210E6` 워치독과의 비유는
  "저빈도 주기적 점검"이라는 개념 차원에서만 유효하고, 실행 방식
  (전용 스레드 유무)까지 같은 건 아니다 - 혼동 방지를 위해 이 절에서
  더 이상 그 비유는 쓰지 않는다.
- 매 깨어남마다: (a) **항상** 작은 고정 배치만큼 aging 스캔(§7.2
  2/3/4단계 - [2026-09-21 3차 정정, QU-02EF5205] PTE를 직접 walk해
  하드웨어 Accessed 비트를 읽고 반영+클리어 → 승격/강등 - "읽기
  전용+#PF" 트랩은 별개인 CoW 기법(`SP-6BEAE0C1`)이라 여기 적용 안 함)을
  전진시킨다(전체 리스트를 한 번에 다 훑지 않음 - 배치 크기는 실측 후
  조정), (b)
  `PageFrameAllocator::freePageCount()`가 `Low` 워터마크 밑이면
  `gInactiveList` tail부터 §7.2 5단계(회수)를 `High` 워터마크에
  도달할 때까지(또는 이번 깨어남의 배치 상한까지) 반복한다.
- **고갈 시점(alloc() 실패)의 역할**: 동기적으로 회수를 트리거하지
  않는다(위 8-1.1 원칙) - 다만 `Low` 워터마크보다 훨씬 낮은
  "긴급(critical)" 문턱 밑으로 떨어졌다는 사실만 다음 콜백 실행에서
  더 공격적으로(배치 상한 없이 `High`까지) 반응하도록 콜백이 자신의
  다음 재등록 간격을 짧게 당기는 것까지는 고려할 수 있다 - 이건
  순수 최적화라 v1 필수는 아니다(실측 후 판단).

### 8-1.4 왜 이 정도로 충분한가 - 회수 쪽도 "즉시 nullptr" 철학과 일관됨

이 설계에서 **`alloc()`은 여전히 회수를 전혀 기다리지 않는다** -
스캔 콜백이 미리 만들어 둔 여유분이 부족하면 `alloc()`은 그냥
`SP-D7013B26` §2.4 그대로 즉시 `nullptr`을 반환한다(회수가 못
따라잡는 상황 자체는 이 설계의 범위 밖 - 그 경우의 정책(OOM kill 등)은
별도 결정 대상). 즉 이 스캔 콜백은 "평소에 미리 여유를 만들어 두는"
보조 역할일 뿐, `PageFrameAllocator`의 핵심 비블로킹 계약은 전혀
바뀌지 않는다.

### 8-1.5 이번 제안이 확정하지 않는 것 (실측 후 결정)

- 정확한 Low/High 워터마크 비율.
- 한 번 깨어날 때의 aging 스캔 배치 크기 / 회수 배치 상한.
- 깨어남 기본 주기(워치독의 "~1초"를 그대로 쓸지, 더 짧게/길게 할지).
- "긴급 문턱"(8-1.3 마지막 항목)의 실제 도입 여부.

이 다섯 가지는 실제 swap 착수 시점(`SP-2BCE5D60`/`PN-452FF696` fs
서비스가 실코드를 갖춘 뒤, 진짜 메모리 압박 워크로드로 계측 가능해질
때) 구현 세션이 계측하며 정한다 - 새 DC 없이 진행 가능한 "숫자 튜닝"
수준으로 판단한다(RM-23F4B687 §4).

## 8-2. [열린 확인 — 설계자] 여전히 남은 것 (구조/스레드는 위 §8-1로 해소)

1. **캐시 타입 불일치 처리**(§3) - `PN-81223433`으로 계속 추적.
2. **FileBacked write-back 경로/dirty 판정**(§7.4) - "공유 LRU를
