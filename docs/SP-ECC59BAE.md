...
**[확정, 2026-09-16, QU-FAC822D4 답변]** "그래, 일단은 (C)로 가자.
RT 자체가 필요할지 안필요할지도 지금 불투명해보이니까." - **(C)
채택**: 자동 발동 트리거는 v1에 두지 않는다. §2-4의 메커니즘만
구현하고, `Scheduler::requestForcedMigration()`은 수동/진단 API로만
노출한다. RT Task를 이 메커니즘의 실제 대상으로 삼을지(§5.1의 §6
항목4 "RT 제외" 원칙과의 관계)를 포함한 정책 결정 전체가 보류됐다 -
RT 클래스 자체의 존속 여부(설계자: "RT 자체가 필요할지 안필요할지도
지금 불투명")까지 열려 있으므로, 자동 트리거 정책은 RT 클래스의
실사용 여부가 판가름난 뒤 재검토한다.
