...
### 3.1 워크 스틸링 (ADR-053, M11 ADR-137로 범위 정정)

**밴드 우선순위(kernel이 항상 user보다 우선, ADR-014)가 노드 경계보다
먼저 적용된다** — ADR-137이 scheduler.md 최초 판의 "kernel 밴드는
노드를 넘지 않는다"는 제약을 폐기했다. 실제 선택 순서:

1. 내 노드의 `kernel_band`.
2. (비었으면) 다른 노드의 `kernel_band`를 순서대로 확인(스틸링).
3. (전부 비었으면) 내 노드의 `user_band`.
4. (비었으면) 다른 노드의 `user_band`를 순서대로 확인(스틸링).

이 협조적 스케줄러(타이머 없음)에는 "일정 시간 유휴"를 측정할
수단이 없어, ADR-053이 언급한 **임계치는 사실상 0**이다 — 위 단계에서
확인할 차례가 된 큐가 비어 있으면 즉시 다음 단계/노드로 넘어간다.

- 훔쳐온 스레드의 `preferred_node`는 원래 값을 그대로 유지한다 —
  스틸링은 일회성 구제일 뿐 영구 이주가 아니다. 다음 스케줄링
  기회에 자기 노드로 돌아갈 수 있다.
- 노드 탐색 순서는 노드 번호 순 라운드로빈이다(SLIT 거리 기반 순서는
  아직 스케줄러 쪽에 반영하지 않았다 — memory.md의 물리 할당자
  폴백만 ADR-054로 거리 기반 순서를 쓴다).
- M1~M10 실행 환경(노드 1개, ADR-035)에서는 훔쳐올 다른 노드가
...
1개짜리 전통적 스케줄러와 같다. M11(`smp-fpu-bringup.md`)이 §3.1(워크
스틸링, ADR-053/137)을 실제로 구현하고 `MINICORE_QEMU_NUMA`로 QEMU
검증했다 — 그 전까지는 노드가 항상 1개라 §3.1 자체가 코드로 존재하지
않았다.
