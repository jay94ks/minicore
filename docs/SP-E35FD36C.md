...
<!--
  ...
  status: approved
  updatedAt: 2026-09-24T12:28:28.017Z
...
**[정정, 2026-09-24, minicore-3c 세션] 아래 2026-09-15 답변 자체는
여전히 유효하나, 그 실행 메커니즘("devmgr이 매칭 성공 시 스폰하는
자식 프로세스")은 devmgr이 이후 KernelThread로 흡수되며 뒤집혔다** -
devmgr 자신이 `SP-43331889`/`PN-615C48D5`(2026-09-20/21 완료)로
Process 없는 순수 커널 `KernelThread`가 되며 `libmc`(트랩 기반
syscall 왕복)를 더 이상 안 쓴다 - `mc::fork()`를 호출할 유저랜드
트랩 컨텍스트 자체가 사라졌다. **이 USB 드라이버가 실제로 착수될
때는 아래 원문(§3.2/§4a-3 기반 fork() 설계)이 아니라
`SP-43331889` §5의 `kSpawnUserModeDriver()`(devmgr이 커널 모드에서
직접 ring3 Process를 만드는 함수, `PN-A8BE8BED` 항목1로 추적)를
따라야 한다** - 자세한 경위는 `SP-9DD4F3EA`(PnP 프레임워크)의
동일 정정 절 참고(이 문서와 같은 devmgr 자식 프로세스 전제를
공유하므로 함께 정정). "격리된 별도 프로세스에서 실행"이라는
목표 자체는 바뀌지 않았다 - 그 프로세스를 누가/어떻게 만드는지만
바뀌었다.

**[갱신, 2026-09-15, SP-9DD4F3EA §3.2/§4a-3 답변 반영, 위 정정 참고]**
이 계층 전체는 devmgr **자신의 프로세스가 아니라**, devmgr이 매칭 성공 시
스폰하는 별도 **자식 프로세스** 안에서 실행된다(설계자 지시 -
"devmgr 인스턴스는 무조건 1개고 그 밑에 자식 프로세스들이 달려야
하는 구조로만 설계해야 해") - 코드는 devmgr 실행 파일에 정적
링크돼 있지만 실행 시점엔 격리된 별도 프로세스다. 논리 계층도
(아래)는 그대로 유효하다.
