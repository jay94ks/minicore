...
<!--
  ...
  status: approved
  updatedAt: 2026-09-17T17:24:15.924Z
...
**[경고, 2026-09-17, 설계자 지시 - 근거 확정됨] 안전하지 않음 - 실 구현에 적용 금지.**
구현 자체는 `PN-DAE91888`(commit c0f35a0)로 이미 존재하고 TEMP/QEMU
검증은 통과했지만, 설계자가 "LockFreeVector는 안전하지 않다고
마킹하고 실 구현에 적용하지 마"라고 지시했고, 후속 확인(QU-09D55D1A)
답변으로 **정확한 근거가 확정됐다**: **세그먼트 할당이 실패했을 때
`pushBack`이 `_size.fetchAdd(1)`로 이미 예약해 버린 전역 인덱스를
되돌릴 방법이 없어 그 자리가 영구 "구멍"으로 남는 문제**(구현
자체가 "커널 메모리 실제 고갈 시에만 발생하는 v1 범위의 트레이드
오프"로 이미 문서화해 뒀던 바로 그 한계) - 설계자가 "이게 가장
치명적"이라고 명시적으로 확인했다. 커널 컨테이너에서 할당 실패는
드문 예외가 아니라 실제로 마주치는 조건이라(유저 메모리 압박 등),
이 결함은 이론적 엣지 케이스가 아니라 실사용 시 실제로 컨테이너를
영구 손상시킬 수 있는 진짜 안전성 문제로 격상됐다. **이 설계/구현을
다른 서브시스템에 실제로 연결(wire-in)하지 않는다** - 이미 존재하는
헤더/코드는 참고용으로만 남기고, 새 소비자가 이걸 쓰려는 시도가
보이면 이 경고를 근거로 저지한다. 재설계한다면 할당 실패 시 그
인덱스를 안전하게 회수/재사용하거나, 애초에 실패가 불가능하도록
사전 확보(reservation) 전략을 쓰는 방향이 필요할 것으로 보이나,
그 재설계 자체는 이 문서 범위 밖(설계자 추가 지시 대기).
...
**[정정, 2026-09-18, RM-F2DAFF66 설계공백 검수] 위 §3 서두 산문
("트리 변경은 새 서브트리를 먼저 구성한 뒤 마지막에 원자적 포인터
1회 교체로 게시하는 방식이어야" - RCU식 copy-then-republish, 아래
설계자 답변의 (B))과 바로 위 코드 스케치(`find()`를 "락 없음"이라고만
적고 재시도 로직이 전혀 없음 - 실제로는 (A)도 (B)도 아닌 무방비
상태)가 서로 다른 이야기를 하고 있었고, 실제 구현(`PN-A8EF29F7`,
`minicore/kernel/concurrent_rbtree.h`)은 **이 문서에 없는 세 번째
방식**으로 확정됐다 - `QU-B5CA4008`(착수 세션이 코드 감사로
`RbCore::rotateLeft/rotateRight`가 원자적 포인터 교체가 아니라 CLRS
표준 in-place 다중 필드 mutate임을 발견하고 등록한 질의)에서 설계자가
명시적으로 "(A) 읽기 seqlock류 검증/재시도"를 선택했다 - 위 §3의
"새 서브트리 구성 후 원자적 교체"(=선택되지 않은 (B))는 채택되지
않았다. 실제 구현은 버전 카운터(홀수=쓰기 중/짝수=고요)를 두고
`find`/`first`/`next`가 시작 전후 버전을 비교해(변경 있었으면
`kMaxRetries=64` 재시도) 순수 `Rbtree::find` 등을 그대로 재사용한다
- `_tree.insert()`/`remove()`(회전 포함) 자체는 전혀 수정하지 않는다
(이 클래스 위 코드 스케치의 `insert()`는 실제로 정확히 이 형태 그대로
구현됐다 - 스케치와 일치하는 부분). `SP-B1E258D8`(RCU)는 이 최종
설계에서는 트리 변경 알고리즘 자체가 아니라 `RcuReadGuard`로 읽기
도중의 선점만 막는 보조 용도로만 쓰인다(원안이 전제한 "노드 회수
안전성을 위한 RCU"라는 이유와는 다르다 - 이 컨테이너는 `Rbtree`와
동일하게 `T`의 메모리를 전혀 소유/회수하지 않는 완전 침습형이라
애초에 그런 회수 자체가 없다). 이 방식이 감내하는 잔여 위험(순회
진행 "도중" 회전이 끼어들면 극히 드물게 일시적 순환을 밟아 그
시도가 안 끝날 이론적 가능성)은 `concurrent_rbtree.h` 자신의 클래스
주석에 정직하게 기록돼 있다. **원문(위 산문+코드 스케치)은 보존하되,
실제 채택된 것은 이 각주가 설명하는 seqlock 방식이라는 점을 명시**
- 다음에 이 문서를 참고하는 세션이 "원자적 교체를 구현해야 한다"고
오인하지 않도록 한다.
...
**[각주, 2026-09-18] 위 두 문장도 실제 구현 기준으로는 정확도가
떨어진다** - 실제로 `ConcurrentRbtree`가 `PN-495C11B7`(RCU)를 필요로
하는 이유는 "노드 회수 안전성"이 아니라 위 각주가 설명한 대로 순수
"읽기 도중 이 코어의 선점을 막는" 보조 용도이고, `ConcurrentMap`의
체인 삭제도 실제로는 "§1과 같은 마킹 방식"이 아니라 스트라이프 락을
쥔 채 직접 unlink한 뒤 `Rcu::callAfterGracePeriod()`로 반납을 미루는
방식이다(`concurrent_map.h` 참고) - 두 컨테이너 모두 §1(`LockFreeList`)
의 Harris류 논리적 마킹 기법을 실제로는 채택하지 않았다.
