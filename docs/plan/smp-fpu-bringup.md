...
- **lazy FPU 전환(CR0.TS 트랩)** — ADR-127이 v1으로 eager를 확정했다.
  필요성이 실측으로 확인되면 별도 ADR.
- **XSAVE/AVX 지원** — 지금 컴파일러가 AVX를 쓰지 않으므로 불필요
  (ADR-127 근거).
...
M9/M10/M11 확인 문자열을 마일스톤 완료 시점마다 추가한다. SMP/NUMA
검증(M10/M11)은 기본 스모크 테스트 경로와 분리한다 —
`MINICORE_QEMU_SMP`/QEMU `-numa`가 opt-in이므로, 이를 쓰는 검증은
별도 스크립트(가칭 `tools/smoke-test-smp-x86_64.sh`)로 만들어 기존
1코어 기본 경로(`smoke-test-x86_64.sh`)를 그대로 보존한다.

## 완료 후

각 마일스톤(또는 M9~M11 전체) 완료 시 `docs/done/`에 결과를 기록한다.
