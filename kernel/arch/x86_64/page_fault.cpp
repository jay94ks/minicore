...
bool try_handle_cow_write_fault_for(uint64_t pml4_phys, uint64_t fault_addr, uint64_t error_code) {
    ...
    // mm::k_page_size가 uint32_t라 그냥 ~(k_page_size-1)을 하면 32비트
    ...
    uint64_t page_virt = fault_addr & ~(static_cast<uint64_t>(mm::k_page_size) - 1);
    ...
    // 뒀다).
    auto new_page = mm::alloc_pages(0, 0);
    if (!new_page.is_ok()) {
        klog::printf("[pf] cow copy failed: out of memory virt=0x%lx\n",
                     static_cast<unsigned long>(page_virt));
        return false;
    }
    uint64_t new_phys = new_page.value();
    __builtin_memcpy(mm::phys_to_virt(new_phys), mm::phys_to_virt(q.phys), mm::k_page_size);

    unmap_page(pml4_phys, page_virt);
    map_page(pml4_phys, page_virt, new_phys, final_perm);
    klog::printf("[pf] cow copy virt=0x%lx old_phys=0x%lx new_phys=0x%lx\n",
                 static_cast<unsigned long>(page_virt), static_cast<unsigned long>(q.phys),
