...
`docs/` 문서 트리, ADR/OPEN 항목 현황, plan→done 진행 상태를 한 화면에서
보고 답변을 `docs/reply.md`에 기록할 수 있는 로컬 웹 도구가 있습니다
(표준 라이브러리만 사용, 별도 설치 불필요):

```bash
python tools/docs-dashboard/server.py
```

실행 후 브라우저에서 `http://127.0.0.1:8765/`에 접속합니다.
