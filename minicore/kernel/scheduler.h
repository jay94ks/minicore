...
public:
    ...
    // 캡처해 진행 상황을 보존하는 것" - `kIsrHandler`(idt.cpp)가
    // 벡터별 분기(EOI, 타이머 처리, 페이지폴트 등)를 시작하기 **전**,
    // 오직 이 인터럽트가 중첩되지 않은(=현재 코어의 인터럽트 깊이가
    // 1인, 진짜로 어떤 Task를 직접 인터럽트한) 경우에만 호출한다 -
    // `frame`(지금 막 하드웨어+isr_common_stub이 만든 InterruptFrame)
    // 을 그대로 `currentTask()->tcb`로 복사해 둔다. 이렇게 하면
    // `onTick()`/`onForcedMigration()`이 나중에(EOI~디스패치 결정
    // 사이 어느 시점에) 재진입되거나 지연되더라도, "이 Task가 어디서
    // 멈췄는지"는 이미 인터럽트 진입 즉시 정확하게 기록돼 있어 그
    // 이후의 스케줄링 결정이 이 값을 다시 캡처할 필요가 없다(단순
    // 참조만 하면 됨) - PN-584DB994가 추적해 온 "gCurrentTask 상태
    // 불일치로 잘못된 tcb가 캡처되는" 클래스의 경쟁을 원천적으로
    // 없애는 게 목표. **중첩 인터럽트에서는 절대 호출하면 안 된다** -
    // 그 경우 `frame`은 원래 Task의 진짜 재개 지점이 아니라 "바깥쪽
    // 인터럽트 처리 도중 어딘가"를 가리켜, 오히려 tcb를 커널 내부
    // 지점으로 오염시킨다(kIsrHandler 호출부의 인터럽트 깊이 확인이
    // 이 호출 자체를 막아 준다).
    static void captureCurrentFrame(InterruptFrame* frame);
