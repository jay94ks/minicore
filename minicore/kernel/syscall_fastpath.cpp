...
extern "C" kernel::uint64_t kSyscallFastDispatch(kernel::uint64_t verb, kernel::uint64_t arg0,
    ...
    // [수정, 2026-09-21, PN-1DFCB337; 갱신, SP-A252E82F] `syscall`
    // 빠른 경로는 isr_common_stub을 거치지 않아 진짜 InterruptFrame이
    // 없다(일반 디스패치 스택 스왑도 안 함) - null을 넘겨
    // self-terminate류 분기가 예전 sti+hlt 방식을 그대로 쓰게 한다
    // (syscall.h의 kDispatchSyscallVerb 문서 주석 참고).
