...
// [신규, 2026-09-25, PN-5481287C 준비 6단계] FAT12/16 폭에 맞춘
// 엔트리 해석 - kInterpretFatEntry(위, FAT32 32비트)와 같은 판별을
// vfat.h의 폭별 EOC/bad-cluster 상수로 재현한다.
ChainStep kInterpretFat16Entry(FatEntryWidth width, uint16_t raw, uint32_t* outNext) {
    const uint16_t eocMin = (width == FatEntryWidth::Fat12) ? kFat12EocMin : kFat16EocMin;
    const uint16_t badCluster = (width == FatEntryWidth::Fat12) ? kFat12BadCluster : kFat16BadCluster;
    if (raw == 0 || raw == badCluster) {
        return ChainStep::Invalid;
    }
    if (raw >= eocMin) {
        return ChainStep::End;
...
// [신규, 2026-09-25, PN-5481287C 준비 6단계] 클러스터 하나의 FAT12/16
// 엔트리가 있는 섹터+그 섹터 안에서의 바이트 오프셋(순수 계산) -
// kFatEntryLocation(위, FAT32)과 같은 역할. FAT12는 1.5바이트 단위라
// 엔트리가 섹터 경계를 걸칠 수 있다(`kFat12EntryByteOffset` 문서
// 주석 참고) - 호출부가 이 섹터부터 반드시 2섹터를 읽어야 안전하다
// (FAT16은 절대 안 걸치지만 같은 2섹터 읽기를 그대로 써도 무해 -
// FAT 영역 뒤엔 항상 루트 디렉터리 영역이 있어(rootEntryCount>0)
// 마지막 FAT 섹터에서 2섹터를 읽어도 장치 끝을 넘지 않는다).
void kFat16EntryLocation(uint32_t fatStartSector, uint32_t bytesPerSector, FatEntryWidth width, uint32_t cluster,
                          uint32_t* outSector, uint32_t* outByteOffsetInSector) {
    const uint32_t absOffset = (width == FatEntryWidth::Fat12) ? kFat12EntryByteOffset(cluster) : cluster * 2u;
    *outSector = fatStartSector + absOffset / bytesPerSector;
    *outByteOffsetInSector = absOffset % bytesPerSector;
}

// [신규, 2026-09-25, PN-5481287C 준비 6단계] kFat16EntryLocation이
// 가리킨 섹터부터 읽은 2섹터짜리 버퍼에서 폭에 맞게 엔트리 값을
// 뽑는다(순수 계산, I/O 없음) - `kFat12EntryGet`/`kFat16EntryGet`
// (vfat.h)은 FAT 영역 시작(클러스터 0)부터의 절대 오프셋을 가정해
// (내부에서 `cluster+cluster/2`/`cluster*2`를 직접 계산) 이런 "섹터
// 두 개짜리 창"에는 그대로 못 쓴다(가상의 음의 오프셋 포인터가
// 되어버림) - 그래서 이미 안 대신 "그 창 안에서의 상대 오프셋"을
// 받는 버전을 별도로 둔다(비트 연산 자체는 vfat.h의 두 함수와 동일).
uint16_t kReadFat16EntryFromWindow(FatEntryWidth width, const uint8_t* twoSectorBuf, uint32_t byteOffsetInSector,
                                    uint32_t cluster) {
    if (width == FatEntryWidth::Fat12) {
        if (cluster & 1) {
            return static_cast<uint16_t>((twoSectorBuf[byteOffsetInSector] >> 4) |
                                          (twoSectorBuf[byteOffsetInSector + 1] << 4));
        }
        return static_cast<uint16_t>(twoSectorBuf[byteOffsetInSector] |
                                      ((twoSectorBuf[byteOffsetInSector + 1] & 0x0F) << 8));
    }
    return static_cast<uint16_t>(twoSectorBuf[byteOffsetInSector] | (twoSectorBuf[byteOffsetInSector + 1] << 8));
}

// [신규, 2026-09-26, PN-C03ED4BC] kReadFat16EntryFromWindow의 쓰기
// 버전(순수 계산, I/O 없음) - `kFat12EntryPut`/`kFat16EntryPut`(vfat.h)
// 과 동일한 비트 연산을 상대 오프셋 버전으로 재현(같은 이유로 절대
// 오프셋 버전을 그대로 못 씀). FAT12는 대상 바이트의 "반대쪽 니블"을
// 보존해야 한다 - 짝수 클러스터는 다음 클러스터(홀수)의 엔트리 상위
// 니블이, 홀수 클러스터는 이전 클러스터(짝수)의 엔트리 하위 니블이
// 같은 바이트를 공유하므로.
void kWriteFat16EntryToWindow(FatEntryWidth width, uint8_t* twoSectorBuf, uint32_t byteOffsetInSector,
                               uint32_t cluster, uint16_t value) {
    if (width == FatEntryWidth::Fat12) {
        if (cluster & 1) {
            twoSectorBuf[byteOffsetInSector] =
                static_cast<uint8_t>((value << 4) | (twoSectorBuf[byteOffsetInSector] & 0x0F));
            twoSectorBuf[byteOffsetInSector + 1] = static_cast<uint8_t>(value >> 4);
        } else {
            twoSectorBuf[byteOffsetInSector] = static_cast<uint8_t>(value & 0xFF);
            twoSectorBuf[byteOffsetInSector + 1] =
                static_cast<uint8_t>((twoSectorBuf[byteOffsetInSector + 1] & 0xF0) | (value >> 8));
        }
    } else {
        twoSectorBuf[byteOffsetInSector] = static_cast<uint8_t>(value & 0xFF);
        twoSectorBuf[byteOffsetInSector + 1] = static_cast<uint8_t>(value >> 8);
    }
}
...
// 공용 읽기 전용(v1) onExec. Fat32Driver와 같은 평탄화 관례(파일 상단
// 문서 주석)를 따르되, 위 vfat_driver.h 문서 주석의 두 차이(루트 고정
// 섹터 범위, 폭별 FAT 엔트리)를 Open/Stat/Readdir 세 곳에 반영한다.
// Write/Mkdir/Rmdir/Unlink는 이번 증분 범위 밖(항상 PermissionDenied -
// KernelFsWriteArgs 문서 주석의 "읽기 전용 뷰" 관례와 동일).
// ===========================================================================

bool Fat16Driver::mount(fs::BlockDevice* device, bool readOnly) {
    if (!volume_.mount(device)) {
        return false;
    }
    mounted_ = true;
...
bool Fat16Driver::remount(bool writable) {
    readOnly_ = !writable;
    return true;
}

kernel::AsyncExecCoro Fat16Driver::onExec(kernel::AsyncTask*, void* argsRaw) {
    const auto op = *static_cast<const kernel::KernelFsOpCode*>(argsRaw);
    fs::BlockDevice* device = volume_.device();
    const uint32_t bytesPerSector = volume_.bytesPerSectorValue();
    const uint32_t sectorsPerCluster = volume_.sectorsPerClusterValue();
    ...
    const uint32_t fatStartSector = volume_.fatStartSectorValue();
    const uint32_t dataStartSector = volume_.dataStartSectorValue();
    const uint32_t rootDirStartSector = volume_.rootDirStartSectorValue();
    const uint32_t rootDirSectorCount = volume_.rootDirSectorCountValue();
    const uint32_t rootBytes = rootDirSectorCount * bytesPerSector;
    const FatEntryWidth width = volume_.entryWidth();

    switch (op) {
        case kernel::KernelFsOpCode::Open: {
            auto* args = static_cast<kernel::KernelFsOpenArgs*>(argsRaw);
            // [vfat_driver.h 문서 주석] 루트는 클러스터가 없다 - firstCluster=0
            // +isDir=true를 "지금 루트를 보고 있다"는 내부 센티널로 쓴다
            // (실제 FAT 스펙에서도 ".." 엔트리가 부모가 루트일 때 cluster=0을
            // 쓰는 것과 같은 관례라 우연이 아니다).
            bool curIsRoot = true;
            ...
            while (pos < args->relPathLen && !failed) {
                ...
                ResolvedEntry matched;
                bool foundInThisDir = false;
                uint32_t scanEntryCluster = 0;  // matched.entryCluster로 옮길 값(루트면 의미 없음)
                ...
                if (curIsRoot) {
                    ...
                    scanEntryCluster = 0;  // 루트 자신은 클러스터가 없음 - entryCluster/ByteOffset 갱신 안 함(entryValid=false 유지)
                } else {
                    uint32_t scanCluster = curCluster;
                    while (true) {
                        uint32_t sector = 0;
                        if (!kClusterToSector(dataStartSector, sectorsPerCluster, scanCluster, &sector)) {
                            break;
                        ...
                        uint32_t nextCluster = 0;
                        if (kInterpretFat16Entry(width, raw, &nextCluster) != ChainStep::Next) {
                            break;
                        }
                        scanCluster = nextCluster;
                    }
                ...
                currentFileSize = matched.fileSize;
                currentEntryValid = true;
                currentEntryCluster = scanEntryCluster;
                currentEntryByteOffset = matched.entryByteOffset;
            }
            ...
            if (slotIndex == kMaxOpenHandles) {
                ...
                OpenHandleEntry& slot = openHandles_[slotIndex];
                slot.inUse = true;
                slot.firstCluster = curCluster;  // 루트면 0(센티널) - 위 문서 주석
                slot.fileSize = currentFileSize;
                slot.isDir = currentIsDir;
                slot.entryValid = currentEntryValid;
                slot.entryCluster = currentEntryCluster;
                slot.entryByteOffset = currentEntryByteOffset;
                args->result = kernel::OpenResult{kernel::FileHandle{slotIndex}, currentIsDir, kernel::VfsError::None};
            }
            break;
        }
        ...
        case kernel::KernelFsOpCode::Read: {
            // 파일 데이터는(루트든 서브디렉터리든 상관없이) 항상 평범한
            // 클러스터 체인이다 - 루트 고정 섹터 범위는 디렉터리 엔트리
            // 저장 방식일 뿐 일반 파일엔 적용되지 않는다(파일이 루트
            // 자신일 수는 없음). Fat32Driver의 Read와 동일한 구조,
            // FAT 엔트리 조회만 폭별 헬퍼로 교체.
            auto* args = static_cast<kernel::KernelFsReadArgs*>(argsRaw);
            const uint32_t handleIndex = static_cast<uint32_t>(args->handle.value);
            if (handleIndex >= kMaxOpenHandles || !openHandles_[handleIndex].inUse) {
                args->result = kernel::ReadResult{0, kernel::VfsError::InvalidHandle};
            ...
            while (clusterStartOffset + bytesPerCluster <= args->offset) {
                uint32_t fatSector = 0;
                uint32_t byteOffsetInSector = 0;
                kFat16EntryLocation(fatStartSector, bytesPerSector, width, cluster, &fatSector, &byteOffsetInSector);
                SlabBuf fatBuf(2 * bytesPerSector);
                if (!fatBuf) {
                    chainBroken = true;
                    break;
                ...
                const uint16_t raw = kReadFat16EntryFromWindow(width, fatBuf.get(), byteOffsetInSector, cluster);
                uint32_t nextCluster = 0;
                if (kInterpretFat16Entry(width, raw, &nextCluster) != ChainStep::Next) {
                    chainBroken = true;
                    break;
            ...
            while (remaining > 0 && !ioFailed) {
                ...
                if (remaining > 0) {
                    uint32_t fatSector = 0;
                    uint32_t byteOffsetInSector = 0;
                    kFat16EntryLocation(fatStartSector, bytesPerSector, width, cluster, &fatSector,
                                         &byteOffsetInSector);
                    SlabBuf fatBuf(2 * bytesPerSector);
                    if (!fatBuf) {
                        ioFailed = true;
        ...
        case kernel::KernelFsOpCode::Write: {
            // [구현, 2026-09-26, PN-C03ED4BC] Fat32Driver의 Write(PN-9D6FE4B6)와
            // 같은 4단계 구조(기존 체인 길이 확인 → 부족하면 free
            // 클러스터 할당·링크 → 청크 단위 read-modify-write → 디렉터리
            // 엔트리 갱신) - FAT12/16 전용으로 두 가지만 다르다: (1) FAT
            // 엔트리 조회/갱신은 전부 폭별 2섹터 창 헬퍼를 통한다, (2)
            // FSInfo(FAT32 전용)가 없어 매번 클러스터 2부터 선형 스캔하고
            // 힌트 캐시를 남기지 않는다(PN-C03ED4BC 계획이 명시한 v1
            // 단순화 - 파일이 root 자신일 수는 없으므로 이 단계엔 루트
            // 특수 케이스가 없다, 4단계의 엔트리 갱신에서만 필요).
            auto* args = static_cast<kernel::KernelFsWriteArgs*>(argsRaw);
            const uint32_t handleIndex = static_cast<uint32_t>(args->handle.value);
            if (handleIndex >= kMaxOpenHandles || !openHandles_[handleIndex].inUse) {
                args->bytesWritten = 0;
            ...
            const uint64_t writeEnd = args->offset + args->len;
            const uint32_t numFats = volume_.numFatsValue();
            const uint32_t fatSizeSectors = volume_.fatSizeSectorsValue();
            const uint32_t clusterCount = volume_.clusterCountValue();

            bool failed = false;
            kernel::VfsError failReason = kernel::VfsError::InvalidHandle;

            // 1단계: 기존 체인 길이와 tail 클러스터.
            uint32_t existingClusterCount = 0;
            uint32_t tailCluster = 0;
            uint32_t firstCluster = h.firstCluster;
            if (firstCluster != 0) {
                ...
                for (;;) {
                    uint32_t fatSector = 0;
                    uint32_t byteOffsetInSector = 0;
                    kFat16EntryLocation(fatStartSector, bytesPerSector, width, c, &fatSector, &byteOffsetInSector);
                    SlabBuf fatBuf(2 * bytesPerSector);
                    if (!fatBuf) {
                        failed = true;
                        break;
                    ...
                    const uint16_t raw = kReadFat16EntryFromWindow(width, fatBuf.get(), byteOffsetInSector, c);
                    uint32_t next = 0;
            ...
            // 2단계: writeEnd를 담기 모자라면 free 클러스터를 할당·링크.
            uint64_t existingCapacity = static_cast<uint64_t>(existingClusterCount) * bytesPerCluster;
            SlabBuf zeroBuf(bytesPerCluster);
            if (!zeroBuf) {
                args->bytesWritten = 0;
            ...
            while (!failed && existingCapacity < writeEnd) {
                uint32_t candidate = kFirstDataCluster;
                bool foundFree = false;
                uint32_t newCluster = 0;
                uint32_t loadedFatSector = 0xFFFFFFFFu;
                SlabBuf scanBuf(2 * bytesPerSector);
                if (!scanBuf) {
                    failed = true;
                    failReason = kernel::VfsError::NoSpace;
                    break;
                }
                for (uint32_t attempts = 0; attempts < clusterCount; ++attempts, ++candidate) {
                    uint32_t fatSector = 0;
                    uint32_t byteOffsetInSector = 0;
                    kFat16EntryLocation(fatStartSector, bytesPerSector, width, candidate, &fatSector,
                                         &byteOffsetInSector);
                    if (fatSector != loadedFatSector) {
                        fs::BlockIoResult scanIoResult;
                        kernel::AsyncTask* scanIoTask =
                            kSubmitReadSectors(device, bytesPerSector, fatSector, 2, scanBuf.get(), &scanIoResult);
                        if (!scanIoTask) {
                            failed = true;
                            failReason = kernel::VfsError::NoSpace;
                            break;
                    ...
                    const uint16_t raw = kReadFat16EntryFromWindow(width, scanBuf.get(), byteOffsetInSector, candidate);
                    if (raw == 0) {
                        foundFree = true;
                        newCluster = candidate;
                        break;
                ...
                // 새 클러스터를 EOC로 먼저 표시(모든 FAT 사본), 그 다음
                // 이전 tail을 이 클러스터로 링크 - Fat32Driver와 동일한
                // 순서(중간 상태 최소화).
                for (uint32_t fatIndex = 0; fatIndex < numFats && !failed; ++fatIndex) {
                    uint32_t sector = 0;
                    uint32_t byteOffsetInSector = 0;
                    kFat16EntryLocation(fatStartSector + fatIndex * fatSizeSectors, bytesPerSector, width, newCluster,
                                         &sector, &byteOffsetInSector);
                    SlabBuf buf(2 * bytesPerSector);
                    if (!buf) {
                        failed = true;
                        failReason = kernel::VfsError::NoSpace;
                        break;
                    ...
                    fs::BlockIoResult readResult;
                    kernel::AsyncTask* readTask =
                        kSubmitReadSectors(device, bytesPerSector, sector, 2, buf.get(), &readResult);
                    if (!readTask) {
                        failed = true;
                        failReason = kernel::VfsError::NoSpace;
                        break;
                    ...
                    const uint16_t eocValue =
                        (width == FatEntryWidth::Fat12) ? kFat12EocMin : kFat16EocMin;
                    kWriteFat16EntryToWindow(width, buf.get(), byteOffsetInSector, newCluster, eocValue);
                    fs::BlockIoResult writeResult;
                    kernel::AsyncTask* writeTask =
                        kSubmitWriteSectors(device, bytesPerSector, sector, 2, buf.get(), &writeResult);
                    if (!writeTask) {
                        failed = true;
                        failReason = kernel::VfsError::NoSpace;
                        break;
                ...
                if (existingClusterCount > 0) {
                    for (uint32_t fatIndex = 0; fatIndex < numFats && !failed; ++fatIndex) {
                        uint32_t sector = 0;
                        uint32_t byteOffsetInSector = 0;
                        kFat16EntryLocation(fatStartSector + fatIndex * fatSizeSectors, bytesPerSector, width,
                                             tailCluster, &sector, &byteOffsetInSector);
                        SlabBuf buf(2 * bytesPerSector);
                        if (!buf) {
                            failed = true;
                            failReason = kernel::VfsError::NoSpace;
                        ...
                        kWriteFat16EntryToWindow(width, buf.get(), byteOffsetInSector, tailCluster,
                                                  static_cast<uint16_t>(newCluster));
                        fs::BlockIoResult writeResult;
                        kernel::AsyncTask* writeTask =
                            kSubmitWriteSectors(device, bytesPerSector, sector, 2, buf.get(), &writeResult);
                        if (!writeTask) {
            ...
            // 3단계: offset이 속한 클러스터까지 체인을 따라간 뒤 청크
            // 단위 read-modify-write.
            uint32_t cluster = firstCluster;
            uint64_t clusterStartOffset = 0;
            bool chainBroken = false;
            while (clusterStartOffset + bytesPerCluster <= args->offset) {
            ...
            // 4단계: fileSize/firstCluster/수정 시각을 메모리+디스크
            // 디렉터리 엔트리 양쪽에 반영 - entryCluster==0이면 그
            // 엔트리는 고정 루트 버퍼 안에 있다(파일 자신은 루트일 수
            // 없으므로 이 0은 항상 "루트에 있는 파일" 센티널, Open()의
            // 문서 주석과 동일한 관례).
            const uint64_t newFileSize = (writeEnd > h.fileSize) ? writeEnd : h.fileSize;
            if (totalWritten > 0 && h.entryValid) {
                const FatTimestamp writeTs = kCurrentFatTimestamp();
                uint32_t entrySector = 0;
                bool haveEntrySector = false;
                if (h.entryCluster == 0) {
                    entrySector = rootDirStartSector;
                    haveEntrySector = true;
                } else {
                    haveEntrySector = kClusterToSector(dataStartSector, sectorsPerCluster, h.entryCluster, &entrySector);
                }
                if (haveEntrySector) {
                    entrySector += h.entryByteOffset / bytesPerSector;
                    const uint32_t byteInSector = h.entryByteOffset % bytesPerSector;
        ...
        case kernel::KernelFsOpCode::Mkdir: {
            // [구현, 2026-09-26, PN-C03ED4BC] Fat32Driver의 Mkdir(PN-9D6FE4B6/
            // PN-3D39A53C/PN-740005DF)과 같은 구조(부모 탐색 → 이름 중복/
            // 빈 슬롯 탐색 → 필요하면 부모 확장 → 새 디렉터리 자신의
            // 클러스터 할당+"."/".." 기록 → 부모에 새 엔트리 기록) -
            // FAT12/16 고유의 새 문제 하나: 부모가 루트일 수 있고, 루트는
            // `Fat16Volume::rootDirSectorCountValue()`로 크기가 고정돼
            // 있어 "빈 슬롯이 없으면 새 클러스터를 이어붙인다"는 일반
            // 디렉터리의 확장 전략을 쓸 수 없다 - 루트가 꽉 차면 정직하게
            // NoSpace(PN-C03ED4BC 계획이 명시한 대로).
            auto* args = static_cast<kernel::KernelFsMkdirArgs*>(argsRaw);
            if (readOnly_) {
                args->error = kernel::VfsError::PermissionDenied;
                break;
            ...
            const uint32_t neededSlots = needsLfn ? (lfnSlotCount + 1) : 1;
            const uint32_t containerEntries =
                (parentIsRoot ? rootBytes : bytesPerCluster) / static_cast<uint32_t>(sizeof(DirEntry));
            if (neededSlots > containerEntries) {
                args->error = kernel::VfsError::InvalidArgument;
                break;
            }
            ...
            bool alreadyExists = false;
            bool freeSlotFound = false;
            bool freeSlotIsRoot = parentIsRoot;
            uint32_t freeSlotCluster = 0;
            uint32_t freeSlotByteOffset = 0;
            bool ioFailed = false;
            uint32_t parentTailCluster = parentCluster;

            auto scanContainerForSlot = [&](uint8_t* buf, uint32_t bufBytes) {
                ResolvedEntry dummy;
                const DirScanResult nameScan = kScanDirClusterForName(buf, bufBytes, leafNormalized,
                                                                       args->relPath + leafStart, leafLen, &dummy);
                if (nameScan == DirScanResult::Found) {
                    alreadyExists = true;
                }
                const uint32_t entriesInBuf = bufBytes / static_cast<uint32_t>(sizeof(DirEntry));
                const auto* entries = reinterpret_cast<const DirEntry*>(buf);
                uint32_t runLength = 0;
                uint32_t runStartOffset = 0;
                for (uint32_t i = 0; i < entriesInBuf; ++i) {
                    const uint8_t firstByte = static_cast<uint8_t>(entries[i].name[0]);
                    const bool isFree = (firstByte == kNameDeletedMarker || firstByte == kNameFreeRestMarker);
                    if (isFree) {
                        if (runLength == 0) {
            ...
            if (parentIsRoot) {
                ...
                if (!rootBuf) {
                    ...
                    if (!ioTask) {
                        ...
                        if (!ioResult.ok) {
                            ...
                            scanContainerForSlot(rootBuf.get(), rootBytes);
                        }
                    }
                }
            } else {
                ...
                while (true) {
                    ...
                    const bool freeSlotFoundBefore = freeSlotFound;
                    const DirScanResult nameScan = scanContainerForSlot(clusterBuf.get(), bytesPerCluster);
                    if (!freeSlotFoundBefore && freeSlotFound) {
                        // 이번 클러스터에서 처음으로 필요한 만큼의 연속
                        // 빈 슬롯을 찾았다 - 그 위치를 기억(이후 클러스터를
                        // 더 훑어도 alias 충돌 검사만 계속할 뿐 이 값은
                        // 덮어쓰지 않는다).
                        freeSlotCluster = scanCluster;
                    }
                    if (nameScan == DirScanResult::Found || alreadyExists) {
                        break;
                    }
                    if (nameScan == DirScanResult::EndOfDir) {
                        break;
            ...
            if (!freeSlotFound) {
                // [vfat_driver.h/PN-C03ED4BC 문서 주석] 루트는 고정
                // 섹터 범위라 새 클러스터를 이어붙여 확장할 수 없다 -
                // 일반 디렉터리와 달리 여기서 정직하게 NoSpace.
                if (parentIsRoot) {
                    args->error = kernel::VfsError::NoSpace;
                    break;
                }
                ...
                const uint32_t clusterCount = volume_.clusterCountValue();
                {
                    ...
                    if (!scanBuf) {
                        ...
                        for (uint32_t attempts = 0; attempts < clusterCount && !extAllocFailed; ++attempts, ++candidate) {
                            uint32_t fatSector = 0;
                            uint32_t byteOffsetInSector = 0;
                            kFat16EntryLocation(fatStartSector, bytesPerSector, width, candidate, &fatSector,
                                                 &byteOffsetInSector);
                            if (fatSector != loadedFatSector) {
                                fs::BlockIoResult scanIoResult;
                                kernel::AsyncTask* scanIoTask = kSubmitReadSectors(
                                    device, bytesPerSector, fatSector, 2, scanBuf.get(), &scanIoResult);
                                if (!scanIoTask) {
                                    extAllocFailed = true;
                                    break;
                                }
                            ...
                            const uint16_t raw =
                                kReadFat16EntryFromWindow(width, scanBuf.get(), byteOffsetInSector, candidate);
                            if (raw == 0) {
                                foundFree = true;
                                extCluster = candidate;
                                break;
                ...
                const uint32_t numFats = volume_.numFatsValue();
                const uint32_t fatSizeSectors = volume_.fatSizeSectorsValue();
                const uint16_t eocValue = (width == FatEntryWidth::Fat12) ? kFat12EocMin : kFat16EocMin;
                for (uint32_t fatIndex = 0; fatIndex < numFats && !extLinkFailed; ++fatIndex) {
                    uint32_t sector = 0;
                    uint32_t byteOffsetInSector = 0;
                    kFat16EntryLocation(fatStartSector + fatIndex * fatSizeSectors, bytesPerSector, width, extCluster,
                                         &sector, &byteOffsetInSector);
                    SlabBuf buf(2 * bytesPerSector);
                    if (!buf) {
                        extLinkFailed = true;
                    ...
                    kWriteFat16EntryToWindow(width, buf.get(), byteOffsetInSector, extCluster, eocValue);
                    fs::BlockIoResult writeResult;
                    kernel::AsyncTask* writeTask =
                        kSubmitWriteSectors(device, bytesPerSector, sector, 2, buf.get(), &writeResult);
                    if (!writeTask) {
                ...
                if (!extLinkFailed) {
                    for (uint32_t fatIndex = 0; fatIndex < numFats && !extLinkFailed; ++fatIndex) {
                        ...
                        uint32_t byteOffsetInSector = 0;
                        kFat16EntryLocation(fatStartSector + fatIndex * fatSizeSectors, bytesPerSector, width,
                                             parentTailCluster, &sector, &byteOffsetInSector);
                        SlabBuf buf(2 * bytesPerSector);
                        if (!buf) {
                            extLinkFailed = true;
                            break;
                        ...
                        kWriteFat16EntryToWindow(width, buf.get(), byteOffsetInSector, parentTailCluster,
                                                  static_cast<uint16_t>(extCluster));
                        fs::BlockIoResult writeResult;
                        kernel::AsyncTask* writeTask =
                            kSubmitWriteSectors(device, bytesPerSector, sector, 2, buf.get(), &writeResult);
                        if (!writeTask) {
            ...
            uint32_t newCluster = 0;
            bool allocFailed = false;
            const uint32_t clusterCountForAlloc = volume_.clusterCountValue();
            {
                uint32_t candidate = kFirstDataCluster;
                bool foundFree = false;
                uint32_t loadedFatSector = 0xFFFFFFFFu;
                ...
                if (!scanBuf) {
                    ...
                    for (uint32_t attempts = 0; attempts < clusterCountForAlloc && !allocFailed;
                         ++attempts, ++candidate) {
                        uint32_t fatSector = 0;
                        uint32_t byteOffsetInSector = 0;
                        kFat16EntryLocation(fatStartSector, bytesPerSector, width, candidate, &fatSector,
...

            {
                ...
                memset(dotEntries[1].ext, ' ', 3);
                dotEntries[1].attr = kAttrDirectory;
                // [FAT12/16 스펙 관례, Fat32Driver와 동일] 부모가 루트면
                // ".."의 firstCluster는 0(다른 드라이버와의 호환 관례) -
                // FAT12/16은 애초에 루트가 클러스터 자체가 없으므로 이
                // 분기가 훨씬 자연스럽다.
                const uint32_t dotDotCluster = parentIsRoot ? 0 : parentCluster;
                dotEntries[1].fstClusHi = static_cast<uint16_t>(dotDotCluster >> 16);
                dotEntries[1].fstClusLo = static_cast<uint16_t>(dotDotCluster & 0xFFFFu);
                dotEntries[1].fileSize = 0;
                dotEntries[1].crtDate = mkdirTs.date;
            ...
            {
                uint32_t entryClusterSector = 0;
                bool haveSector = freeSlotIsRoot
                                       ? (entryClusterSector = rootDirStartSector, true)
                                       : kClusterToSector(dataStartSector, sectorsPerCluster, freeSlotCluster,
                                                           &entryClusterSector);
                if (!haveSector) {
                    args->error = kernel::VfsError::NoSpace;
                    break;
                }
                const uint32_t bufBytes = freeSlotIsRoot ? rootBytes : bytesPerCluster;
                const uint32_t bufSectors = freeSlotIsRoot ? rootDirSectorCount : sectorsPerCluster;
                ...
                fs::BlockIoResult readResult;
                kernel::AsyncTask* readTask = kSubmitReadSectors(device, bytesPerSector, entryClusterSector,
                                                                   bufSectors, entryClusterBuf.get(), &readResult);
                if (!readTask) {
                    args->error = kernel::VfsError::NoSpace;
                    break;
                }
                ...
                fs::BlockIoResult writeResult;
                kernel::AsyncTask* writeTask = kSubmitWriteSectors(device, bytesPerSector, entryClusterSector,
                                                                     bufSectors, entryClusterBuf.get(), &writeResult);
                if (!writeTask) {
                    args->error = kernel::VfsError::NoSpace;
                    break;
                }
        ...
        case kernel::KernelFsOpCode::Rmdir: {
            // [구현, 2026-09-26, PN-C03ED4BC] Unlink와 거의 동일한 구조
            // (부모 탐색 → leaf 위치 확인 → 삭제 마킹 → 클러스터 반납) -
            // 다른 점 둘(Fat32Driver의 Rmdir/Unlink 관계와 동일): (1)
            // 대상이 디렉터리여야 함, (2) 삭제 전 "."/".." 외의 유효
            // 엔트리가 없는지 확인 - 합성 불가 제약(파일 상단 문서
            // 주석)으로 Unlink와 공유 못 해 그대로 중복.
            auto* args = static_cast<kernel::KernelFsRmdirArgs*>(argsRaw);
            if (readOnly_) {
                args->error = kernel::VfsError::PermissionDenied;
                break;
...

            {
                ...
                bool haveSector = leafInRoot
                                       ? (entryClusterSector = rootDirStartSector, true)
                                       : kClusterToSector(dataStartSector, sectorsPerCluster,
                                                           leafMatched.entryCluster, &entryClusterSector);
                if (!haveSector) {
                    args->error = kernel::VfsError::InvalidHandle;
                    break;
                }
...

            {
                ...
                while (c != 0) {
                        ...
                        if (!fatBuf) {
                            ...
                            if (!fatIoTask) {
                                ...
                                if (!fatIoResult.ok) {
                                    ...
                                    const uint16_t raw =
                                        kReadFat16EntryFromWindow(width, fatBuf.get(), byteOffsetInSector, c);
                                    step = kInterpretFat16Entry(width, raw, &nextC);
                                }
                            }
                        }
                    }
                    for (uint32_t fatIndex = 0; fatIndex < numFats && !freeFailed; ++fatIndex) {
                        uint32_t sector = 0;
                        uint32_t byteOffsetInSector = 0;
                        kFat16EntryLocation(fatStartSector + fatIndex * fatSizeSectors, bytesPerSector, width, c,
                                             &sector, &byteOffsetInSector);
                        SlabBuf buf(2 * bytesPerSector);
                        if (!buf) {
                            freeFailed = true;
                        ...
                        kWriteFat16EntryToWindow(width, buf.get(), byteOffsetInSector, c, 0);
                        fs::BlockIoResult writeResult;
                        kernel::AsyncTask* writeTask =
                            kSubmitWriteSectors(device, bytesPerSector, sector, 2, buf.get(), &writeResult);
                        if (!writeTask) {
        ...
        case kernel::KernelFsOpCode::Unlink: {
            // [구현, 2026-09-26, PN-C03ED4BC] 대상의 부모 디렉터리를
            // 먼저 찾고(루트일 수 있음 - Open()/Stat()과 같은 특수
            // 케이스), 그 안에서 leaf 이름의 엔트리 위치와 firstCluster를
            // 찾아 (1) name[0]을 0xE5로 마킹, (2) 클러스터 체인을 전부
            // 반납한다. 디렉터리는 거부(Rmdir 전용).
            auto* args = static_cast<kernel::KernelFsUnlinkArgs*>(argsRaw);
            if (readOnly_) {
                args->error = kernel::VfsError::PermissionDenied;
                break;
        ...
        case kernel::KernelFsOpCode::Readdir: {
            ...
            const OpenHandleEntry& dirHandle = openHandles_[dirHandleIndex];
            // [vfat_driver.h 문서 주석] firstCluster==0 && isDir==true는
            // Open()이 남긴 "이건 루트다" 센티널.
            const bool dirIsRoot = dirHandle.isDir && dirHandle.firstCluster == 0;
            ...
            if (dirIsRoot) {
                ...
                if (!ioTask) {
                    ...
                    if (!ioResult.ok) {
                        ...
                        const DirScanResult scanResult =
                            kScanDirClusterForIndex(rootBuf.get(), rootBytes, args->index, &seen, args->entry.name,
                                                     sizeof(args->entry.name), &args->entry.nameLength,
                                                     &args->entry.isDirectory);
                        found = (scanResult == DirScanResult::Found);
                    }
                }
            } else {
                uint32_t cluster = dirHandle.firstCluster;
                SlabBuf clusterBuf(bytesPerCluster);
                if (!clusterBuf) {
                    args->hasMore = false;
                    args->error = kernel::VfsError::InvalidHandle;
                ...
                while (!found && !ioFailed) {
                    ...
                    const DirScanResult scanResult =
                        kScanDirClusterForIndex(clusterBuf.get(), bytesPerCluster, args->index, &seen,
