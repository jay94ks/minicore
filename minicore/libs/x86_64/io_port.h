...
inline void kOutB(uint16_t port, uint8_t value) {
...
inline void kOutW(uint16_t port, uint16_t value) {
...
inline void kOutL(uint16_t port, uint32_t value) {
    asm volatile("outl %0, %1" : : "a"(value), "Nd"(port));
}

inline uint32_t kInL(uint16_t port) {
    uint32_t value;
